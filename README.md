...
- **It paints its own background.** The canvas is opaque (white in light mode, near-black in dark) and clipped to a circle — it isn't a transparent overlay.
- **Dark mode** follows the `dark` class on `<html>`, and only when `forceLight` is `false`.
