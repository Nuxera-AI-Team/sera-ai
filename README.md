...
Sera AI ships three recorder components, each suited to a different use case:
...
Plus [`AudioVisualizerImproved`](#audiovisualizerimproved), the animated canvas the recorders use, exported separately for hosts that build their own UI on top of the `useAudioRecorder` hook.
...
The animated canvas visualization rendered by `AudioRecorder` and `AudioCapture` while recording — a circular frequency spectrum with particle effects, falling back to an idle animation when there's no signal.

### Overview

It's exported on its own so a host that drives capture through the headless [`useAudioRecorder`](#exports) hook can show the same visualization without adopting a recorder component's UI. It only reads the stream you hand it — it never starts, stops, or holds onto capture.

### Basic Usage

```tsx
import { useAudioRecorder, AudioVisualizerImproved } from 'sera-ai';

function Recorder() {
  const recorder = useAudioRecorder({ apiKey: 'your-key', speciality: 'general' });

  return (
    <div>
      <AudioVisualizerImproved
        mediaStream={recorder.mediaStreamRef.current}
...
| Prop | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| `mediaStream` | `MediaStream \| null` | Yes | — | Live capture stream to analyse. `null` runs the idle animation. |
| `isRecording` | `boolean` | Yes | — | Drives the spectrum vs. idle animation. Pass `false` while paused. |
| `forceLight` | `boolean` | No | `true` | Ignore the host's dark mode and always render the light theme. |
| `className` | `string` | No | — | Classes for the outer container — this is where you set the size. |
| `background` | `string` | No | — | Surface painted behind the animation: any CSS color, or `'transparent'`. Omit to follow the host's light/dark theme. |

### Theming the surface

By default the visualizer paints an opaque disc — white, or near-black when the host has the `dark` class on `<html>` and `forceLight` is `false`. Hosts on a theme of their own should say so explicitly:

```tsx
{/* Match a fixed dark panel */}
<AudioVisualizerImproved background="#0A0A0A" mediaStream={stream} isRecording />

{/* Or draw straight onto whatever is behind it */}
<AudioVisualizerImproved background="transparent" mediaStream={stream} isRecording />
```

`'transparent'` gives the canvas an alpha channel and stops both the canvas and the disc behind it from painting, so the animation composites over the host's own background.

### Notes

- **Dark mode** follows the `dark` class on `<html>`, and only when `forceLight` is `false`. `background` overrides both.
- **Tailwind consumers:** the container uses Tailwind utility classes. Tailwind v4 doesn't scan `node_modules`, so add `@source "../node_modules/sera-ai/dist";` to your CSS entry if those classes aren't already generated by your own markup.
- **Audio graph lifecycle:** it opens one `AudioContext` over the stream while recording and closes it when recording stops or the component unmounts — safe to mount for the lifetime of a long-running page.
...
export { AudioVisualizerImproved } from 'sera-ai';

// Hooks — headless recorder for hosts that bring their own UI
export { useAudioRecorder } from 'sera-ai';
...
export type { AudioVisualizerImprovedProps } from 'sera-ai';
export type { AudioRecorderHookProps, UseAudioRecorderReturn } from 'sera-ai';
```
